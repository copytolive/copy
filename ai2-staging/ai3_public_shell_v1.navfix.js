...
function primary(nav){const h=find(nav,["Backtest","Home"]),f=find(nav,["Fundamental"]),c=find(nav,["Screener Crypto","Crypto"]);if(!h||!f||!c)return;nav.dataset.ai3Primary="1";const hl=label(h);if(hl)hl.textContent="Home";h.title="Home";const cl=label(c);if(cl)cl.textContent="Crypto";c.title="Crypto";const s=add(nav,f,"signal","Signal Scan",sig,()=>direct("scanner-signal"));f.insertAdjacentElement("afterend",s);if(s.nextElementSibling!==c)s.insertAdjacentElement("afterend",c);const x=find(nav,["Hyperliquid"])||add(nav,f,"hyper","Hyperliquid",hyp,()=>direct("live-trading"));if(c.nextElementSibling!==x)c.insertAdjacentElement("afterend",x)}
