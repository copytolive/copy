...
function install(){
  ...
  function repaint(){if(state.suppress)return;state.suppress=true;try{st.setMode(st.mode||window._historyTypeFilter||'all');}catch(e){}requestAnimationFrame(function(){requestAnimationFrame(function(){state.suppress=false;});});}
