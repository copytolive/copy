...
function important(el,k,v){if(!el||!el.style)return false;try{if(el.style.getPropertyValue(k)===v&&el.style.getPropertyPriority(k)==='important')return false;el.style.setProperty(k,v,'important');return true;}catch(e){return false;}}
function attr(el,k,v){if(!el)return false;try{if(el.getAttribute(k)===v)return false;el.setAttribute(k,v);return true;}catch(e){return false;}}
function findTradeTitle(){var panel=document.getElementById('backtestHistoryPanel');if(!panel)return null;var nodes=panel.querySelectorAll('div,span');for(var i=0;i<nodes.length;i++){if(String(nodes[i].textContent||'').trim().indexOf('📋 Detailed Trade Log · ALL PAIRS')===0)return nodes[i];}return document.getElementById('ctlDetailedTradeLogTitleV24');}
function stabilizeTradeLog(){var panel=document.getElementById('backtestHistoryPanel');if(panel){important(panel,'display','block');important(panel,'visibility','visible');important(panel,'opacity','1');important(panel,'animation','none');important(panel,'transition','none');important(panel,'transform','none');}var title=findTradeTitle();if(title){if(title.id!=='ctlDetailedTradeLogTitleV24')title.id='ctlDetailedTradeLogTitleV24';if(title.textContent!=='📋 Detailed Trade Log · ALL PAIRS')title.textContent='📋 Detailed Trade Log · ALL PAIRS';important(title,'display','block');important(title,'visibility','visible');important(title,'opacity','1');important(title,'animation','none');important(title,'transition','none');important(title,'transform','none');}}
function hideRetired(){
  var rr=document.getElementById('rrMonitorBox');if(rr){important(rr,'display','none');attr(rr,'aria-hidden','true');}
  var rt=document.getElementById('renkoSmaHistoryTable')||document.getElementById('renkoSmaHistoryEmpty')||document.getElementById('renkoSmaHistoryRows');if(rt){var box=rt.closest&&rt.closest('.workspace-renko-history,.section');if(box){important(box,'display','none');attr(box,'aria-hidden','true');}}
  var titles=document.querySelectorAll('.section-title');for(var i=0;i<titles.length;i++){var t=String(titles[i].textContent||'').trim();if(t.indexOf('RENKO SMA10')>=0&&t.indexOf('LIVE HISTORY')>=0){var b=titles[i].closest&&titles[i].closest('.workspace-renko-history,.section');if(b){important(b,'display','none');attr(b,'aria-hidden','true');}}}
  if(document.body&&document.createTreeWalker){var w=document.createTreeWalker(document.body,NodeFilter.SHOW_TEXT),node,guard=0;while((node=w.nextNode())&&guard++<12000){var raw=String(node.nodeValue||'');if(raw.indexOf('HL CACHE ACCOUNT')>=0){var clean=raw.replace(/HL CACHE ACCOUNT[^\n]*/g,'').trim();if(clean!==raw)node.nodeValue=clean;}}}
}
function install(){
