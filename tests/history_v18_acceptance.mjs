...
for(const q of ["frontendOpenOrders","openOrders","historicalOrders","historicalStatusOpen","__CTL_HL_PENDING_REAL_V18__","clearinghouseState","active-open-orders-v18"]) if(!p.includes(q)) throw new Error('pending v18 contract missing '+q);
for(const q of ["mode==='pending'?pendingMetrics():directMetrics","TOTAL ENTRY","PROBABILITAS","RISK/REWARD","userFills-realized","__CTL_HL_PENDING_REAL_V18__"]) if(!s.includes(q)) throw new Error('summary v18 contract missing '+q);
